...
if (typeof window.SUPABASE_ANON_KEY === 'undefined') {
  ...
  window.sbFetch = async function(table, method, body, qs) {
    ...
    // Send the signed-in Firebase user's ID token as the bearer credential
    // (instead of the shared anon key) so Supabase Row Level Security can
    // tell WHO is asking and enforce per-user / admin-only access. Falls
    // back to the anon key for logged-out, public requests (e.g. the
    // services/packages/testimonials pages and lead-capture forms) — RLS
    // policies must explicitly allow whatever those are permitted to do.
    var bearer = SUPABASE_ANON_KEY;
    try {
      if (window.firebase && firebase.auth && firebase.auth().currentUser) {
        bearer = await firebase.auth().currentUser.getIdToken();
      }
    } catch (e) { /* ignore — falls back to anon key */ }
