...
(function() {
  ...
  window.kasSyncDB = async function(u) {
    var key = getKey();
    if (!u || !key || key.includes('PASTE')) return;
    var bearer = key;
    try { bearer = await u.getIdToken(); } catch (e) { /* fall back to anon key */ }
    var h = { 'apikey':key, 'Authorization':'Bearer '+bearer, 'Content-Type':'application/json', 'Prefer':'return=representation' };
