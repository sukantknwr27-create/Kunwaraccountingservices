// ============================================================
// Kunwar Accounting Services — Navigation (Firebase Auth aware)
// ============================================================
...
function injectNav(active) {
  ...
  // Get user from localStorage (set by firebase.js after login)
  let userData = null;
  try { userData = JSON.parse(localStorage.getItem('kas_user_data') || 'null'); } catch(e) {}
  const isLoggedIn = !!userData;
  const uName = userData?.name || userData?.email?.split('@')[0] || 'Account';
  ...
  const accountBtn = isLoggedIn
    ? `<a href="dashboard.html" class="btn-account" title="${uName}">
        <div class="btn-account-avatar">${initials}</div>
        ${uName.split(' ')[0]}
       </a>`
    : `<a href="login.html" class="btn-account">
        <div class="btn-account-avatar">👤</div>
        Login
       </a>`;
  ...
  const mobileLinks = pages.map(p => `<a href="${p.href}">${p.label}</a>`).join('') +
    (isLoggedIn
      ? `<a href="dashboard.html">👤 My Account</a>
         <a href="#" onclick="kasNavLogout()" style="color:#DC2626;">Logout</a>`
      : `<a href="login.html">👤 Login / Sign Up</a>`) +
    `<a href="payment.html" style="color:var(--gold);font-weight:600;">💳 Pay Now</a>
     <a href="tools.html">🛠️ Free Tools</a>
  ...
  document.getElementById('footer-placeholder').innerHTML = `
    <footer>
      <div class="footer-links">
        ...
        <a href="compliance-calendar.html">Tax Calendar</a>
        ${isLoggedIn ? '<a href="dashboard.html">My Account</a>' : '<a href="login.html">Login</a>'}
      </div>
      <p>
        <strong>Kunwar Accounting Services</strong><br>
        Dwarka, New Delhi &nbsp;·&nbsp;
        ...
        © ${new Date().getFullYear()} Kunwar Accounting Services. All rights reserved.
      </p>
    </footer>

    <a href="https://wa.me/918076136300?text=Hi!%20I%20want%20to%20book%20a%20free%20consultation."
       target="_blank" rel="noopener" id="wa-float" title="Chat on WhatsApp">
  ...
  // Firebase auth state listener — updates nav dynamically
  if (typeof window !== 'undefined') {
    import('./firebase.js').then(({ onUserChanged }) => {
      onUserChanged(user => {
        const accountEl = document.querySelector('.btn-account');
        if (!accountEl) return;
        if (user) {
          const n = user.displayName || user.email?.split('@')[0] || 'Account';
          const init = n.split(' ').map(w=>w[0]).join('').toUpperCase().slice(0,2);
          accountEl.href = 'dashboard.html';
          accountEl.innerHTML = `<div class="btn-account-avatar">${init}</div>${n.split(' ')[0]}`;
        } else {
          accountEl.href = 'login.html';
          accountEl.innerHTML = `<div class="btn-account-avatar">👤</div>Login`;
        }
      });
    }).catch(() => {});
  }
...
// Global logout function called from mobile nav
window.kasNavLogout = async function() {
  try {
    const { kasSignOut } = await import('./firebase.js');
    await kasSignOut();
  } catch(e) {
