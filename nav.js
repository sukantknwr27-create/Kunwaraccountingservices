...
function injectNav(active) {
  ...
  var navLinks = pages.map(function(p) {
    return '<li><a href="'+p.href+'"'+(p.id===active?' class="active"':'')+'>'+p.label+'</a></li>';
  }).join('');
  ...
  var accountBtn = isLoggedIn
    ...
    : '<a href="login.html" class="btn-account"><div class="btn-account-avatar">👤</div>Login</a>';

  var mobileLinks = pages.map(function(p) { return '<a href="'+p.href+'">'+p.label+'</a>'; }).join('');
  mobileLinks += isLoggedIn
    ? '<a href="dashboard.html">👤 My Account</a><a href="#" onclick="kasNavLogout()" style="color:#DC2626;">Logout</a>'
    : '<a href="login.html">👤 Login / Sign Up</a>';
  mobileLinks += '<a href="payment.html" style="color:var(--gold);font-weight:600;">💳 Pay Now</a>';
  mobileLinks += '<a href="tools.html">🛠️ Free Tools</a>';
  mobileLinks += '<a href="compliance-calendar.html">📅 Tax Calendar</a>';
  ...
  var footerLinks = pages.map(function(p){return '<a href="'+p.href+'">'+p.label+'</a>';}).join('');
  footerLinks += '<a href="payment.html">Pay Now</a>';
  footerLinks += '<a href="compliance-calendar.html">Tax Calendar</a>';
  footerLinks += isLoggedIn ? '<a href="dashboard.html">My Account</a>' : '<a href="login.html">Login</a>';
...
window.kasNavLogout = function() {
  if (typeof kasSignOut === 'function') {
    kasSignOut().then(function() { window.location.href = 'index.html'; });
  } else {
